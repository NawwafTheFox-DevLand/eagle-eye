...
export default function ApprovalsClient({ items }: { items: PendingItem[] }) {
  ...
  const t = {
    ar: {
      ...
      emptyTitle: 'لا توجد طلبات بانتظار موافقتك',
    ...
    en: {
      ...
      emptyTitle: 'No requests awaiting your approval',
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">{t.subtitle(items.length)}</p>
      </div>
            ...
            {items.map(item => {
              ...
              return (
                <Link key={item.stepId} href={`/dashboard/requests/${item.requestId}`}
                  ...
                  <div className="w-10 h-10 rounded-full bg-amber-100 flex items-center justify-center text-amber-700 shrink-0">⏳</div>
